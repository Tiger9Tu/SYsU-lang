...
public:
    ...
    Type(BaseType _bt, bool _is_ptr, bool _is_func, bool _is_const = 0) :
        base_type(_bt),
        is_ptr(_is_ptr),
        is_func(_is_func),
        is_const(_is_const),
    ...
    bool operator==(const Type &type2) {
        return this->base_type == type2.base_type
               && this->is_ptr == type2.is_ptr
               && this->is_func == type2.is_func
               && this->dims == type2.dims;
...
struct ImplicitCastExpr
    ...
    ImplicitCastExpr(Expr *_son, Type _type) :
        expr_p(_son) {
        this->type = _type;
...
struct DeclRefExpr
    ...
    Expr *resolveImplicitCast(Type _type, ValueCatagory _vc, int _cheat = 0) final {
        ...
        Expr *p = this;
        Type cast;
        if (!(this->type.dims == _type.dims)) { // arrayToPointerDecay
            cast = _type;
            cast.is_ptr = true;
            p = Mgr::g.make<ImplicitCastExpr>(this, cast);
        } else if (!(this->type.is_func == _type.is_func)
                   || !(this->type.is_ptr == _type.is_ptr)) {
            cast = this->type;
...
struct ExprList
    ...
    void resolveImplicitCastList(std::vector<Type> _types, std::vector<Expr::ValueCatagory> _vcs,
                                 std::vector<int> _cheats = std::vector<int>()) { // this is called when there is no father expr

        for (int i = 0; i < this->size(); i++) { // for example: func(a) ,where a is casted to prvalue
            if (_cheats.size() > 0)
...
struct ArraySubscriptExpr
    ...
    ArraySubscriptExpr(int _flag, Expr *_expr_p_l, Expr *_expr_p_r) :
        flag(_flag), expr_p_l(_expr_p_l), expr_p_r(_expr_p_r) {
...
struct BinaryOperator
    ...
    BinaryOperator(std::string _opcode,
                   ...
                   Expr *_expr_p_r) {
        this->opcode = _opcode;
        pro_base_type = (int)(_expr_p_l->type.base_type) > (int)(_expr_p_r->type.base_type) ?
                            _expr_p_l->type.base_type :
                            _expr_p_r->type.base_type;
        if (this->opcode == "=") {
            this->type = _expr_p_l->type;
...
struct FloatingLiteral
    ...
    FloatingLiteral(std::string _serial) :
        serial(_serial) { // maby
...
struct IntegerLiteral
    ...
    IntegerLiteral(std::string _serial) :
        serial(_serial) { // maby
    ...
    void get_value() {
        ...
        if (this->value <= 2147483647 && this->value >= -21474836478) {
            this->type.base_type = Type::INT;
        } else if (this->serial.find("0x") != std::string::npos) { // weried llvm
...
struct ReturnStmt
    ...
    ReturnStmt(int _flag, Expr *_expr_p) :
        flag(_flag), expr_p(_expr_p) {
        resolveImplicitCast();
    }
...
struct DeclStmt
    ...
    DeclStmt(InitDeclList *_init_decl_list_p) :
        init_decl_list_p(_init_decl_list_p) {
...
struct WhileStmt
    ...
    WhileStmt(Expr *_expr_p, Stmt *_stmt_p) :
        expr_p(_expr_p), stmt_p(_stmt_p) {
...
struct DoStmt
    ...
    DoStmt(Expr *_expr_p, Stmt *_stmt_p) :
        expr_p(_expr_p), stmt_p(_stmt_p) {
        this->expr_p = _expr_p->resolveImplicitCast(_expr_p->type, Expr::PRVALUE);
    }
};
...
struct CompoundStmt
    ...
    CompoundStmt(int _flag, StmtList *_stmt_list_p) :
        flag(_flag), stmt_list_p(_stmt_list_p) {
    }
};
...
struct TranslationUnitDecl
    : public Obj,
      public std::vector<ExternalDecl *> {
    int accept(Visitor *pv) final {
        return pv->visit(this);
    }
};
